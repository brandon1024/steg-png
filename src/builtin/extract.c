...
#define DEFLATE_STREAM_BUFFER_SIZE (128 * 1024)
...
int cmd_extract(int argc, char *argv[])
{
	...
	const struct command_option extract_cmd_options[] = {
			OPT_STRING('o', "--output", "file", "alternate output file directory", &output_file),
			OPT_LONG_BOOL("hexdump", "print a hexdump of the embedded data instead of writing to file", &hexdump),
...
static int extract(const char *input_file, const char *output_file, int show_hexdump)
{
	...
	struct strbuf in;
	strbuf_init(&in);
	strbuf_grow(&in, DEFLATE_STREAM_BUFFER_SIZE);

	struct strbuf out;
	strbuf_init(&out);
	strbuf_grow(&out, DEFLATE_STREAM_BUFFER_SIZE);
	...
	while ((has_next_chunk = chunk_iterator_has_next(&ctx)) != 0) {
		...
		if (!memcmp(ctx.current_chunk.chunk_type, chunk_type, CHUNK_TYPE_LENGTH)) {
			ssize_t bytes_read = 0;
			while ((bytes_read = chunk_iterator_read_data(&ctx, (unsigned char *)in.buff, DEFLATE_STREAM_BUFFER_SIZE)) > 0) {
				strm.avail_in = bytes_read;
				strm.next_in = (unsigned char *)in.buff;

				do {
					strm.avail_out = DEFLATE_STREAM_BUFFER_SIZE;
					strm.next_out = (unsigned char *)out.buff;
					...
					size_t have = DEFLATE_STREAM_BUFFER_SIZE - strm.avail_out;
					if (recoverable_write(tmp_fd, strm.next_out, have) != have)
						FATAL("failed to write inflated data to output file");

					strbuf_remove(&out, 0, have);
	...
	if (!show_hexdump || output_file) {
		...
		if (fstat(tmp_fd, &st) && errno == ENOENT)
			FATAL("failed to stat %s'", input_file);

		if (copy_file_fd(out_fd, tmp_fd) != st.st_size)
